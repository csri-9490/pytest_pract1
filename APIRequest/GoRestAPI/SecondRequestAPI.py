...
def post_request():
    ...
    data={"name":"zzccc Chelukala",
          ...
          "gender":"male",
          "status":"active"}
    mydata=open("C:\\Users\\Srikanth Chelukala\\PycharmProjects\\APIRequest\\GoRestAPI\\data.json","r").read()
    post_resp=requests.post(url,data=json.loads(mydata),headers=header)
...
def delete_request(user_id):#expected status code for delete=>204:Not Found
    url = base_url + f'/public/v2/users/{user_id}'
    header = {"Authorization": auth_token}
